...
function init_on_top_button() {
  jQuery('.wrapper').append('<a href="#" id="on_top" class="general_navigation_button"><span>بالای صفحه &#8593;</span></a>');
  jQuery('#on_top').click(function(e) {
    jQuery.scrollTo(0, 500);
    
    e.preventDefault();
  });
  jQuery(window).scroll(function() {
    if(jQuery(window).scrollTop() > jQuery(window).height()) jQuery('#on_top').css('display', 'block');
    else jQuery('#on_top').hide();
  });
}
